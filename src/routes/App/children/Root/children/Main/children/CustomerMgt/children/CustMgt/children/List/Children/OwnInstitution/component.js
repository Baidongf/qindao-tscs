...
class OwnInstitution extends React.Component {
  ...
  handleConfirm(e) {
    ...
    // 整理 checkedKeys
    let institutionIds = []
    checkedKeys.forEach(keyStr => {
      const keyArr = keyStr.split('-')
      institutionIds.push(parseInt(keyArr[keyArr.length - 1]))
    })
    institutionIds = [...new Set(institutionIds)]
    this.props.deliverInstitutionIds(institutionIds)
  ...
  componentWillReceiveProps({ managerTree, managerList, institutionIds }) {
    ...
    // 因为传递到外部的已选institutionIds结构经过加工，所以这里只处理清空已选的情况(外部点击“重置按钮”)
    if (
      institutionIds !== this.props.institutionIds &&
