...
class List extends React.Component {
  ...
  constructor(props) {
    ...
    this.pagination = {
      total: 0,
      current: this.props.manageListFilter.pageNum,
      pageSize: this.props.manageListFilter.pageSize,
      onChange: (page, pageSize) => {
        this.pagination.current = page
        this.props.manageListFilter.pageNum = page
  ...
  publishHandler = (knowlObj) => {
    const id = knowlObj.id
    this.props.publishKnowl(id, this.searchHandler)
  }
  ...
  tableSortHandler(currentPage,pagination, filters, sorter){
    if(sorter&&sorter.field&&this.state.filterObj.filed!==(sorter.field==='viewCount'?1:0)&&!(this.state.filterObj.viewSort===sorter.order||this.state.filterObj.timeSort===sorter.order)){
      ...
      this.setState({
        ...
        this.props.setManageListFilter({...this.state.filterObj,sortField:filed,sortType:type,timeSort,viewSort})
        this.searchHandler()
      })
    }else{
    	if(pagination.current === undefined || (pagination.current===1&&currentPage===1)){
          ...
          this.props.setManageListFilter({...filter})
		      this.searchHandler()
		    })
    	}
		}
  ...
  editHandler = (item) => {
    // this.props.history.push('/root/main/customerKnowlMgt/createOrEdit?operation=edit&id=' + item.id)
    window.location.href="/#/root/main/customerKnowlMgt/createOrEdit?operation=edit&id=" + item.id
  }
  ...
  render() {
    ...
    let columns = [
      ...
      dataIndex: 'title',
      className: 'td-name',
      render: (value, row, index) => <HzLink to={`/root/main/customerKnowlMgt/managerdetail?id=${row.id}`}><span
    ...
    return (
      <div className='home-component knowlMgt-list'>
        <div className='list-component'>
          {/*<HzBreadcrumb/>*/}
          <div style={{height:134}} className='filter-area'>
            <div style={{display: "block", float: "none",height:60}} className='inputs'>
              <Select
          ...
          <div className='result-area'>
            <div className='list-header clearfix'>
              ...
              <div className='btns'>
                {checkPermission("knowledge/save")&&<Link to='/root/main/customerKnowlMgt/createOrEdit?operation=create'>
                  <Button type="primary"
                  >信息创建</Button>
                </Link>}
              </div>
            ...
            <div className='table-area'>
              <div className='hz-table'>
                <Table
                  ...
                  rowKey={'key'}
	                pagination={this.pagination.total > this.props.manageListFilter.pageSize ? this.pagination : false}
                />
              </div>
            </div>
          </div>
